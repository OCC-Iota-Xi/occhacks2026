...
export default function Countdown() {
  ...
  return (
    <div
      ...
      className={`transition-opacity duration-500 ${remaining === null ? "opacity-0" : "opacity-100"}`}
    >
      <div className="flex items-start gap-5 pl-1 sm:gap-7">
        {units.map((unit) => (
          <div key={unit.label} className="flex flex-col items-center">
            <span className="font-header text-3xl tracking-wider text-white drop-shadow-[0_2px_8px_rgba(0,0,0,0.5)] sm:text-4xl md:text-5xl">
              <RollingNumber value={unit.value} max={unit.max} />
            </span>
            <span className="mt-1 text-xs text-muted-foreground sm:text-sm">{unit.label}</span>
          </div>
        ))}
      </div>
    </div>
